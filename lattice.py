...
from htv_utils import generate_custom_square_grid

# TODO: Compare HTV with discrete HTV
...
class Lattice():
    ...
    def __init__(self, X_mat=None, C_mat=None,
                lsize=10, C_init='zero', **kwargs):
        """
        Args:
        ...
        if X_mat is not None and C_mat is not None:
            ...
            assert X_mat == C_mat, 'X_mat is None and C_mat is not, or vice-versa.'
    ...
    @staticmethod
    def init_hexagonal_lattice(lsize=10, C_init='zero', **kwargs):
        """ """
    ...
    def is_hexagonal(self):
        """ """
        if not np.allclose(self.X_mat[1, 0], 0.):
            return False
        if not np.allclose(self.X_mat[0, 1], self.X_mat[0, 0]/2):
            return False
        if not np.allclose(self.X_mat[1, 1], self.X_mat[0, 1]*math.sqrt(3)):
            return False
    ...
    def verify_input(cls, X_mat, C_mat):
        ...
        if X_mat_size != (ldim, ldim):
            raise ValueError(f'Expected size ({ldim},{ldim}). Found size {X_mat_size}.')
        if len(C_mat_size) != ldim or not all(k == C_mat_size[0] for k in C_mat_size):
            raise ValueError(f'Expected size ([k] * {ldim}). Found size {C_mat_size}.')
        if C_mat_size[0] % 2 != 1:
            raise ValueError(f'Expected odd dimensions. Found size {C_mat_size}.')

        # verify consistency
        if X_mat.det().allclose(Tensor([0.])):
            raise ValueError('X_mat is not invertible or very ill-conditioned.')
    ...
    def init_origin_triangles(self):
        ...
        barycentric coordinates of translated x wrt one of the origin triangles.
        (see get_barycentric_mask/coordinates())
        """
        vA, vB, vC, vD = [0, 0], [1, 0], [1, 1], [0, 1] # vertices of origin square
        self.batch_origin_triangles = torch.tensor([[vA, vB, vD],
                                                    [vB, vC, vD]]) # (2,3,2)
        assert self.batch_origin_triangles.size() == (2,3,2)

        # size (2,3,3); for each of the 2 triangles (dim=0), get barycentric matrices
        self.batch_origin_triangles_barycentric_mat = \
            self.append_ones(self.batch_origin_triangles, dim=-1).transpose(1, 2)
        ...
        assert self.batch_inv_origin_triangles_barycentric_mat.size() == (2,3,3)
    ...
    def init_neighborhood(self):
        ...
        # size: (6,2)
        self.neighborhood_diff = torch.tensor([v_Ma, v_Mb, v_Mc, v_A, v_B, v_C])
    ...
    def update_lattice(self, X_mat, C_mat):
        ...
        self.X_mat_inv = torch.inverse(X_mat) # so that inversion does not have to be repeated
        ...
        self.lattice_grid,_,_ = generate_custom_square_grid(self.lmin, self.lmax+1, 1)
        assert self.lattice_grid.size() == ((self.lsize + 1) ** 2, 2)

        # Now we get three corresponding tensors:
        # self.unique_triangles[i] <-> self.triangle_centers[i] <-> self.affine_coeff[i]
        self.init_unique_triangles()
    ...
    def get_lattice_mask(self, x_lat, pad=0):
        ...
        lattice_mask = (x_lat_min >= self.lmin+pad) * (x_lat_max <= self.lmax-pad)
    ...
    def init_unique_triangles(self):
        ...
        x_lat,_,_  = generate_custom_square_grid(self.lmin, self.lmax, 1)

        m = x_lat.size(0)
        x_lat_expand = x_lat.view(m, 1, 1, 2).expand(m, 2, 3, 2)

        unique_triangles_aux = x_lat_expand + self.batch_origin_triangles # (m,2,3,2)
        ...
        unique_triangles_rel = self.unique_triangles.sub(self.lmin) # size (m, 3, 2)

        # values of vertices of each unique triangle
        self.unique_triangles_values = \
            self.C_mat[unique_triangles_rel[:, :, 0], unique_triangles_rel[:, :, 1]]

        assert self.unique_triangles_values.size() == (self.unique_triangles.size(0), 3)
    ...
    def init_triangle_centers(self):
        """ """
        try:
            self.triangle_centers = self.get_triangle_centers(self.unique_triangles)
        except AttributeError:
            print('Need to initialize unique_triangles before triangle_centers.')
            raise


    @classmethod
    def get_triangle_centers(cls, triangles):
        """ Get tensor with centers of lattice triangles in lattice coordinates """
        ...
        xcenter = (triangles_mat @ batch_xcenter_barycentric_coordinates.unsqueeze(-1)).squeeze(-1)
    ...
    def append_ones(cls, x, dim=0):
        ...
        """
        if dim not in [0, -1]:
            raise ValueError('Can only append ones to first or last dimension.')

        x_size = x.size()
        assert x_size[dim] == cls.ldim, ('concatenation dimension should be the '
                                            'same as the lattice dimension...')
    ...
    def get_affine_coefficients(self, x_lat, use_saved_affine=True, **kwargs):
        ...
        Args:
            ...
            use_saved_affine: use saved self.affine_coeff, resulting in a speedup.
        Returns:
        ...
        if use_saved_affine is True:
            try:
                ...
                idx_unique_triangles,_,_ = self.check_my_triangle(x_lat_in, **kwargs)

                affine_coeff = torch.zeros(x_lat.size(0), 3)
                affine_coeff[lattice_mask] = self.affine_coeff[idx_unique_triangles]
    ...
    def get_affine_coeff_from_plane_coeff(plane_coeff):
        ...
        Plane equation: a1*x1 + a2*x2 + z + d = 0 (coefficients (a1, a2, 1, d));
    ...
    def check_my_triangle(self, x_lat, check_method='halfplane', **kwargs):
        ...
        Args:
            x_lat: size(m, 2); batch of m 2-dimensional vectors in lattice basis
        ...
        if not self.inside_lattice(x_lat):
            raise ValueError('check_my_triangle(): x_lat should lie inside lattice.')
        ...
        base_triangle_idx = x_lat_floor[:, 1].long().sub(self.lmin) * (self.lsize * 2) + \
    ...
    def get_halfplane_mask(x_lat_remainder):
        ...
        """
        # {x1} + {x2} - 1 >= 0 -> belongs to second triangle; {xi} - remainder of xi
    ...
    def get_barycentric_mask(self, x_lat_remainder):
        ...
        r_mat = self.append_ones(x_lat_remainder, dim=-1).unsqueeze(-1) # (m,3,1)
    ...
    def get_plane_coefficients(self, x_lat, plane_method='solve', **kwargs):
        ...
        vertices_values_in = \
            self.C_mat[vertices_lat_in_rel[:, :, 0], vertices_lat_in_rel[:, :, 1]]

        vertices_size = vertices_lat_in.size() # (m,3,2)
        vertices_lat_in_reshape = vertices_lat_in.view(-1, 2) # (m*3, 2) to convert all vertices to standard
        vertices_std_in = \
            self.lattice_to_standard(vertices_lat_in_reshape.float()).reshape(vertices_size)

        vertices_std_in_with_values = \
            torch.cat((vertices_std_in, vertices_values_in.unsqueeze(-1)), dim=-1)
        ...
        plane_coeff = torch.tensor([0., 0., 1., 0.]).unsqueeze(0).expand(x_lat.size(0), -1).clone()
    ...
    def normal_method(cls, vertices_std_with_values):
        ...
        For n=2, the normal can be found using the outer product of the triangle
        indexes. If point (x1,x2) lives in triangle A,B,C, the normal (n1,n2,n3)
        can be obtained by doing the cross product ABxAC. After this, we
        normalize according to the third coordinate (n1/n3, n2/n3, 1),
        then choose a point in the plane where (x1,x2) lives and plug
        into the equation d = -(a1.x1 + a2.x2 + z).

        Args:
            vertices_std_with_values: size (m,3,3)
        """
        m = vertices_std_with_values.size(0)
        AB = vertices_std_with_values[:, 1, :] - vertices_std_with_values[:, 0, :] # (m,3)
        AC = vertices_std_with_values[:, 2, :] - vertices_std_with_values[:, 0, :] # (m,3)
        assert AB.size() == (m,3)
        ...
        normalized_normals = normals.div(c.view(-1, 1)) # a1.x1 + a2.x2 + z + d = 0
        assert normalized_normals.size() == (m,3)

        plane_coeff = cls.get_plane_coeff_from_normals_vertices(normalized_normals,
    ...
    def solve_method(cls, vertices_std_with_values):
        ...
        plane equation: a1.x1 +...+ an.xn + z + d = 0; normal: (a1,...,an,1)
        For n = 2, we have 3 vertices and 3 unkowns (a1, a2, d) for each datapoint.
        ...
        z_diff = (vertices_values[:, 0:2] - vertices_values[:, 2:3]).unsqueeze(-1)
    ...
    @staticmethod
    def get_plane_coeff_from_normals_vertices(normalized_normals, vertices_std_with_values):
        """
        Args:
        ...
        d = (normalized_normals * vertices_std_with_values[:, 0, :]).sum(1).mul(-1)
        assert d.size() == (m,)
    ...
    def lattice_to_standard(self, x_lat):
        ...
        xi[0]X_mat[:, 0] + xi[1]X_mat[:, 1] + ... + xi[n]X_mat[:,n] = X_mat @ xi
        """
        assert x_lat.dtype == torch.float32
        assert len(x_lat.size()) == 2, f'x_lat should be 2D'
        assert x_lat.size(1) == self.ldim, f'found size {x_lat.size(1)}...'

        x_standard = (self.X_mat @ x_lat.t()).t()
    ...
    def standard_to_lattice(self, x_standard):
        ...
        The ith output vector corresponds to taking the linear combination
        of the columns of lattice matrix with scalars forming vector xi:
        xi[0]X_mat^(-1)[:, 0] + xi[1]X_mat^(-1)[:, 1] + ... + xi[n]X_mat^(-1)[:,n] = X_mat^(-1) @ xi
        """
        assert x_standard.dtype == torch.float32
        assert len(x_standard.size()) == 2, 'x_standard should be 2D'
        assert x_standard.size(1) == self.ldim, f'found size {x_standard.size(1)}...'

        x_lat = (self.X_mat_inv @ x_standard.t()).t()
    ...
    def get_values_from_interpolation(self, x_lat, **kwargs):
        ...
        Args:
            ...
            vertices_lat: size (m,3,2); vertices of triangle where each data
                        data point is located. 3x2 matrix of -1 if outside lattice;
        Returns:
        ...
        # TODO: Generalize next operation (e.g.) C_mat[vertices_lat_rel.split(dim=-1)]
        vertices_values_in = \
            self.C_mat[vertices_lat_in_rel[:, :, 0], vertices_lat_in_rel[:, :, 1]]

        assert vertices_values_in.size() == (m,3)

        # batch inner product: f(xi) = a1f(v1) + a2f(v2) + a3f(v3), i \in {1,...,m}
        # where v1, v2, v3 are the vertices of the triangle where xi lives
        x_values_in = (vertices_values_in * x_lat_in_barycentric_coordinates).sum(1) # (m,)
        assert x_values_in.size() == (m,)

        vertices_lat = torch.ones((x_lat.size(0), 3, 2), dtype=torch.int64).mul(-1)
    ...
    def get_barycentric_coordinates(self, x_lat, in_origin=True, **kwargs):
        ...
        Returns:
            ...
            vertices_lat: size (m, 3, 2); vertices of triangles where x_lat lives.
        """
        if not self.inside_lattice(x_lat):
            raise ValueError('get_barycentric_coordinates(): x_lat should lie inside lattice.')
        ...
        if in_origin is True:
            ...
            warnings.warn('in_origin=False is poorly conditioned. Prefer in_origin=True.')
            batch_r = self.append_ones(x_lat, dim=-1) # (m,3)
            batch_R_mat = self.append_ones(vertices_lat, dim=-1).transpose(1, 2) # (m,3,3)
    ...
    def get_values_from_affine_coefficients(self, x_lat, **kwargs):
        ...
        # get (a'1, a'2, d')
        al1, al2, dl = affine_coeff[:, 0], affine_coeff[:, 1], affine_coeff[:, 2]
    ...
    def generate_fine_square_grid(self, basis='lattice'):
        """ Generates fine square 2D grid and returns list of (x1,x2)
        grid points.

        Returns:
            x: size (m,2) = (a*a, 2); list of (x1,x2) grid points
            x_vec: vector that originated grid (size (a,)).
            Xsq_size: size of meshgrid tensor (size (a, a)).
        """
        assert basis in ['lattice', 'standard']
        # create random step sampled according to uniform distribution
        unif_avg = 0.003 * self.lsize
        unif_range = 0.0002
        step = np.random.uniform(unif_avg - unif_range, unif_avg + unif_range)

        x_lat, x_vec, Xsq_size = generate_custom_square_grid(self.lmin, self.lmax, step)
        if basis == 'lattice':
            return x_lat, x_vec, Xsq_size

        x_std = self.lattice_to_standard(x_lat)

        return x_std, x_vec, Xsq_size



    def get_lattice_borders(self, pad=0, basis='lattice'):
        """ Returns the lattice borders in lattice or
        standard coordinates, pad distance from the actual border

        Returns:
            tuple (x_min, y_min, x_max, y_max)
        """
        vertices_lat = \
            torch.tensor([[self.lmin+pad, self.lmin+pad],
                        [self.lmax-pad, self.lmax-pad]])

        if basis == 'lattice':
            return tuple(v for v in vertices_lat.flatten().numpy())

        # standard coordinates
        vertices_std = \
            self.lattice_to_standard(vertices_lat.float()).squeeze()

        return tuple(v for v in vertices_std.flatten().numpy())
    ...
    def divide_lattice(self):
        ...
        new_C_mat[0, :] = torch.from_numpy(np.interp(x, xp, self.C_mat[0, :].numpy()))
        new_C_mat[:, 0] = torch.from_numpy(np.interp(x, xp, self.C_mat[:, 0].numpy()))
        new_C_mat[-1, :] = torch.from_numpy(np.interp(x, xp, self.C_mat[-1, :].numpy()))
        new_C_mat[:, -1] = torch.from_numpy(np.interp(x, xp, self.C_mat[:, -1].numpy()))

        # fill interior by piecewise linear interpolation
        int_x_lat, _, int_Xsq_size = \
            generate_custom_square_grid(self.lmin+0.5, self.lmax-0.1, step=0.5)
        int_x_values, _ = self.get_values_from_interpolation(int_x_lat)
        int_new_C_mat = int_x_values.reshape(int_Xsq_size).t()
    ...
    def box_spline(x_lat, **kwargs):
        ...
        zeros_vec, ones_vec = torch.zeros_like(a1), torch.ones_like(a2)
